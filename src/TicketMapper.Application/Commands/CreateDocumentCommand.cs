                    ...
                    using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(request.TicketDetails.FileName, WordprocessingDocumentType.Document))
                    {
                        ...
                        for (int i = request.TicketDetails.StartNumber; i <= request.TicketDetails.EndNumber; i++)
                        {
                            try
                            {
                                ...
                                ticket.Mutate(x =>
                                {
                                    x.DrawText(i.ToString(), font, Color.Black, new PointF(130, 550));
                                });
                            ...
                            {
                                Console.WriteLine($"Error processing ticket {i}: {ex.Message}");
                            }
                        }
                        Console.WriteLine("Tickets created and added to Word document successfully!");
                    }
                }
                catch (Exception ex)
                {
